...
function addButtons(selection, popupNode) {
  ...
  // Add add-to-anki button
  const ankiButton = document.createElement('img');
  ankiButton.className = 'ankiButton';
  ankiButton.title = 'Save this entry to Anki!';
  ankiButton.style = 'position: absolute; z-index: 16777270; max-width: 22px;';
  ankiButton.style.top = rect.bottom + pageYOffset - 30 + 'px';
  ankiButton.style.left = rect.left + pageXOffset + 5 + 'px';
  ankiButton.onclick = () => {
    updateAnki(word, ankiButton);
  };
  updateAnkiIcon(true, ankiButton);
  popupNode.after(ankiButton);
...
function updateAnki(word, ankiButton){
  const messageBackground = browser.runtime.sendMessage({
    word: word,
    context: 'just a test',
    action: 'save_anki'
  });
  messageBackground.then( saved => {
    updateAnkiIcon(saved, ankiButton);
  });
}

function updateAnkiIcon(saved, ankiButton){
  if (saved){
    ankiButton.src = browser.runtime.getURL("images/save-to-anki-success.png");
  } else {
    ankiButton.src = browser.runtime.getURL("images/save-to-anki-failed.png");
  }
}
